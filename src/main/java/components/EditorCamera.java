...
import main.java.engine.KeyListener;
...
public class EditorCamera extends Component {
    ...
    private double dragDebounce = 0.032f;
    ...
    public void editorUpdate(double dt) {
        if (MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_MIDDLE) && dragDebounce > 0) {
            ...
            levelEditorCamera.position.sub(delta.mul((float)dt).mul(dragSensitivity));
            this.clickOrigin.lerp(mousePos, (float)dt);
        }
        ...
        if (reset) {
            ...
            this.lerpTime += 0.1f * (float)dt;
            if (Math.abs(levelEditorCamera.position.x) <= 4.0f && Math.abs(levelEditorCamera.position.y) <= 4.0f) {
                this.lerpTime = 0.0f;
